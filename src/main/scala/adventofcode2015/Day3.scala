...
object Day3 extends App {
  sealed abstract class Sym(val rep: Char)

  case object North extends Sym('^')
  case object South extends Sym('v')
  case object East extends Sym('>')
  case object West extends Sym('<')

  case class Coord(x: Int, y: Int)

  def numHousesDelivered(input: String): Int = {
    val housesVisited: mutable.Set[Coord] = mutable.Set(Coord(0, 0))
    var currCoord = Coord(0, 0)
    var houseCount = 1
    ...
    for (i <- 0 to input.length - 1) {
      ...
      val temp = currCoord
      direction match {
        case North.rep => currCoord = Coord(temp.x, temp.y + 1)
        case South.rep => currCoord = Coord(temp.x, temp.y - 1)
        case East.rep => currCoord = Coord(temp.x + 1, temp.y)
        case West.rep => currCoord = Coord(temp.x - 1, temp.y)
      }

      if (!housesVisited.contains(currCoord)) {
        housesVisited += currCoord
        houseCount += 1
      }
    }
  ...
  private def getNextCoord(c: Char)(currCoord: Coord): Coord = {
    c match {
      case North.rep => Coord(currCoord.x, currCoord.y + 1)
      case South.rep => Coord(currCoord.x, currCoord.y - 1)
      case East.rep => Coord(currCoord.x + 1, currCoord.y)
      case West.rep => Coord(currCoord.x - 1, currCoord.y)
    }
  }

  def numHousesDeliveredTailRec(input: String): Int = {
    @tailrec
    def loop(housesVisited: Set[Coord], currCoord: Coord, index: Int, houseCount: Int): Int = {
      if (index == input.length) return houseCount

      val direction = input(index)
      val nextCoord = getNextCoord(direction)(currCoord)

      if (!housesVisited.contains(nextCoord)) {
        loop(housesVisited + nextCoord, nextCoord, index + 1, houseCount + 1)
      } else {
        loop(housesVisited, nextCoord, index + 1, houseCount)
      }
    }

    loop(Set(Coord(0, 0)), Coord(0, 0), 0, 1)
  }

//  private def unzipString(input: String): (String, String) = {
//   val (list1, list2) = input.toList.zipWithIndex.partition { case (_, i) => i % 2 == 0 }
//
//    (list1.map(_._1).mkString, list2.map(_._1).mkString)
//  }

  def twoSantasHousesDelivered(input: String): Int = {
    @tailrec
    def loop(housesVisited: Set[Coord], currCoords: (Coord, Coord), index: Int, houseCount: Int): Int = {
      if (index == input.length) return houseCount

      val (santaCoord, roboCoord) = currCoords
      val direction = input(index)

      if (index % 2 == 0) {
        val nextCoord = getNextCoord(direction)(santaCoord)
        if (!housesVisited.contains(nextCoord)) {
          loop(housesVisited + nextCoord, (nextCoord, roboCoord), index + 1, houseCount + 1)
        } else {
          loop(housesVisited, (nextCoord, roboCoord), index + 1, houseCount)
        }
      } else {
        val nextCoord = getNextCoord(direction)(roboCoord)
        if (!housesVisited.contains(nextCoord)) {
          loop(housesVisited + nextCoord, (santaCoord, nextCoord), index + 1, houseCount + 1)
        } else {
          loop(housesVisited, (santaCoord, nextCoord), index + 1, houseCount)
        }
      }
    }

    loop(housesVisited = Set(Coord(0,0)), currCoords = (Coord(0,0), Coord(0,0)), index = 0, houseCount = 1)
  }
  ...
  val input2 = "^^^^^"
//  println(twoSantasHousesDelivered(input))
//  println(input.length)
//  println(numHousesDelivered(input2))
//  println(time { numHousesDelivered(input2) })
//  println(time { numHousesDeliveredTailRec(input2) })
//  println(s"input.length: ${input.length}")
  val input3 = ">" * 10000000
  println(time { numHousesDelivered(input3) })
  println(time { numHousesDeliveredTailRec(input3) })
}
